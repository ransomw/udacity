...
def _get_guserinfo(access_token):
...
def gconnect():
    ...
    if (stored_credentials is not None and
        stored_gplus_id == gplus_id
        and stored_credentials.access_token == credentials.access_token):
        response = make_response(
            json.dumps('Current user already connected'), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    # store access creds for later use
    login_session['credentials'] = credentials
    login_session['gplus_id'] = gplus_id
    data = _get_guserinfo(credentials.access_token)
...
def gdisconnect():
    ...
    if credentials is None:
        ...
        return response
    access_token = credentials.access_token
    # data = _get_guserinfo(access_token)
    url = ('https://accounts.google.com/o/oauth2/revoke?token=%s'
           % access_token)
    print "revoke url: ", url
    h = httplib2.Http()
    (result_header, result_body) = h.request(url, 'GET')
    if result_header['status'] == '200':
...
def editRestaurant(restaurant_id):
  ...
  if (user_id is None or
      editedRestaurant.user_id != user_id):
      return redirect(url_for('showRestaurants'))
  if request.method == 'POST':
...
def deleteRestaurant(restaurant_id):
  ...
  user_id = login_session.get('user_id')
  if (user_id is None or
      restaurantToDelete.user_id != user_id):
      return redirect(url_for('showRestaurants'))
  if request.method == 'POST':
